...
// Try to fetch real location data
fetch('https://api.qjqq.cn/api/Local')
    ...
    .catch(error => {
        console.warn('Location API unavailable, using default location:', error);
        // Use default location and show welcome anyway
        showWelcome();
    });
...
function showWelcome() {
    // Safety check for ipLocation
    if (!ipLocation || !ipLocation.data) {
        console.warn('ipLocation data not available');
        return;
    }

    let dist = getDistance(120.082129, 30.302611, ipLocation.data.lng || 120.082129, ipLocation.data.lat || 30.302611);
    let pos = ipLocation.data.country || "未知";
    ...
    try {
        document.getElementById("welcome-info").innerHTML =
            `<span>热烈欢迎来自~</span><br><span><span style="color: var(--icat-card-welcome);font-weight: bold;">${pos}</span> 的学友🍂</span><br><span class="welcome-message">${posdesc}</span><br>您当前位置距博主约 <b><span style="color: var(--icat-card-welcome);font-weight: bold;">${dist}</span></b> 公里！<br><span>您的IP地址为：${ip}</span><br>${timeChange}`;
